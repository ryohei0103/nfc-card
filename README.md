...
├── dashboard.html    マイページ（「プロフィール」「QRコード」「アクセス解析」タブ切り替え）
...
2. `dashboard.html` は「プロフィール」「QRコード」「アクセス解析」の3タブ構成（麻雀アプリの「記録入力／マイページ」と同じタブ切り替えUI）
...
5. 「アクセス解析」タブ: 総閲覧数・直近7日・**NFCタップ数**・**リンククリック数**・連絡先保存数を表示。NFCタグ書き込み時のURLには`?src=nfc`、QRコードには`?src=qr`が自動付与され、公開ページ訪問時にどちらの経由か記録される
