...
6. 「QRコード」→`qr.html`（麻雀アプリと同じタブUIで「QRコードで共有」「QRコードを読み取る」を切り替え）。共有タブでは自分の公開URLのQRコード表示（NFC非対応端末向けの代替共有手段）、読み取りタブではカメラで相手のQRコードを読み取って公開ページへ移動できる（カメラ利用はHTTPS環境が必須）
